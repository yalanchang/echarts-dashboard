...
const stats_get = defineEventHandler(async (event) => {
  ...
  requireAuth(event);
  const [revenue, orders, users, refundRate, revenueByMonth, ordersByWeekday] = await Promise.all([
    // 本月營收
    queryOne(`
        SELECT
  ...
  return ok({
    kpi: {
      revenue: (_a = revenue == null ? void 0 : revenue.total) != null ? _a : 0,
      prevRev: (_b = revenue == null ? void 0 : revenue.prev_total) != null ? _b : 0,
      orders: (_c = orders == null ? void 0 : orders.count) != null ? _c : 0,
      prevOrders: (_d = orders == null ? void 0 : orders.prev_count) != null ? _d : 0,
      activeUsers: (_e = users == null ? void 0 : users.count) != null ? _e : 0,
      refundRate: (_f = refundRate == null ? void 0 : refundRate.rate) != null ? _f : 0
    },
    revenueByMonth,
    ordersByWeekday
  });
...
const export_get = defineEventHandler(async (event) => {
  ...
  const csv = [
    headers.join(","),
    ...rows.map(
      (r) => [r.order_no, r.user_name, r.email, r.total, r.status, r.created_at].join(",")
    )
  ].join("\n");
...
const index_post$4 = defineEventHandler(async (event) => {
  ...
  await queryOne(
    `INSERT INTO orders (user_id, order_no, total, status, note)
     VALUES (?, ?, 0, ?, ?)`,
    [body.user_id, orderNo, (_a = body.status) != null ? _a : "pending", (_b = body.note) != null ? _b : ""]
  );
...
const index_post = defineEventHandler(async (event) => {
  ...
  const result = await queryOne(
    ...
    [body.name, body.email, hash, (_a = body.role) != null ? _a : "viewer"]
  );
